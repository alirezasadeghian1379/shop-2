...
class BluesmindsApiRepository implements IAiRepository
{
    ...
    private function sendRequest(array $messages)
    {
        return Http::withOptions([
            ...
            'Authorization' => 'Bearer '.config('ai.bluesminds.key'),
            'Content-Type' => 'application/json',
            'Accept' => 'application/json',
        ])
            ->acceptJson()
            ->timeout(60)
            ->post(config('ai.bluesminds.url'), [
                'model' => config('ai.bluesminds.model'),
                'messages' => $messages,
                'temperature' => 0.2,
                'max_tokens' => 300,
            ]);
