...
class GrogApiRepository implements IAiRepository
{
    public function chat(string $message, array $history = [],string $type = 'NORMAL',array $restaurant = []): array
    {
        try {
            ...
            report($exception);
            return ['error' => true, 'message' => 'در حال حاضر امکان پاسخ‌گویی وجود ندارد.',];
        }
    }
    ...
    private function sendRequest(array $messages)
    {
        return Http::withOptions([
            ...
            'Authorization' => 'Bearer '.config('ai.grog.key'),
            'Content-Type' => 'application/json',
            'Accept' => 'application/json',
        ])
            ->acceptJson()
            ->timeout(60)
            ->post(config('ai.grog.url'), [
                'model' => config('ai.grog.model'),
                'messages' => $messages,
                'temperature' => 0.2,
    ...
    private function selectedPrompt(string $message, array $history = []): array {
        ...
        // Topic inheritance is for unresolved or explicit follow-up questions.
        ...
        // Explicit video request still receives video on follow-up turns.
        // Do not inherit video merely because it appeared earlier.
        $dependencies = [
            7 => [8, 9],
            8 => [9],
            15 => [14],
